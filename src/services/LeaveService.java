...
public class LeaveService {
    ...
    static {
        ...
        leaveRequests.add(new LeaveRequest(nextId++, 2, "John Doe",   "Sick",
            LocalDate.now().minusDays(10), LocalDate.now().minusDays(9),
            "Fever",        "Approved", LocalDate.now().minusDays(11)));
        leaveRequests.add(new LeaveRequest(nextId++, 3, "Jane Smith", "Casual",
            LocalDate.now().plusDays(3),  LocalDate.now().plusDays(4),
            "Family event", "Pending",  LocalDate.now().minusDays(1)));
        leaveRequests.add(new LeaveRequest(nextId++, 4, "Ravi Kumar", "Earned",
            LocalDate.now().plusDays(7),  LocalDate.now().plusDays(10),
            "Vacation",     "Pending",  LocalDate.now()));
        leaveRequests.add(new LeaveRequest(nextId++, 5, "Priya Nair", "Sick",
            LocalDate.now().minusDays(3), LocalDate.now().minusDays(2),
            "Cold",         "Rejected", LocalDate.now().minusDays(4)));
    }
    ...
    public String applyLeave(LeaveRequest lr) {
        ...
        if (days > available)
            return "Not enough " + lr.getLeaveType() + " leave! Available: " + available;
