...
public class LeaveApproval extends JPanel {
    ...
    private void initUI() {
        ...
        header.add(Theme.label("Leave Approvals", Theme.bold(22), Theme.TEXT_PRIMARY), BorderLayout.WEST);
    ...
    private void populate() {
        ...
        for (LeaveRequest lr : leaveList) {
            ...
            tableModel.addRow(new Object[]{
                lr.getLeaveId(), lr.getEmpName() != null ? lr.getEmpName() : "Emp #" + lr.getEmpId(),
                lr.getLeaveType(), lr.getFromDate(), lr.getToDate(), days, lr.getReason(), lr.getStatus()
            });
        }
    }

    private void handleAction(String action) {
        // Needs a table reference — use parent approach
            ...
            if (c instanceof JPanel) {
                for (Component cc : ((JPanel)c).getComponents()) {
                    if (cc instanceof JScrollPane) {
                        table = (JTable)((JScrollPane)cc).getViewport().getView();
                    }
                }
            }
        }
        ...
        if (row < 0 || row >= leaveList.size()) { JOptionPane.showMessageDialog(this, "Select a leave request."); return; }
